...
class RollingJSONLPipeline:
    ...
    def __init__(
        ...
        self.base_dir = get_setting("RAW_OUT_DIR", "../data/raw/novel")
        ...
        # Open file handles per series (append mode); closed on spider close
        ...
        self.logger.info(
            "Initialized RollingJSONLPipeline base_dir=%s storage_max_gb=%.2f jsonl_name=%s quota_check_interval_s=%.1f quota_item_interval=%d fsync_item_interval=%d",
            str(self.base_dir), self.storage_max_gb, self.jsonl_name,
            self._quota_check_interval_s, self._quota_item_interval, self._fsync_item_interval
        )
    ...
    def _enforce_quota(self):
        used = self._get_dir_size_bytes(self.base_dir)
        self.logger.debug(
    ...
    def _enforce_quota_throttled(self):
        ...
        if (
            self._items_since_quota_check >= self._quota_item_interval
            or (now - self._quota_last_check_ts) >= self._quota_check_interval_s
        ):
            self._enforce_quota()
    ...
    def _open_series_fp(self, jsonl_path: Path) -> gzip.GzipFile:
        ...
        if fp is None:
            # Append to gzip is safe; produces multi-member gzip which readers can handle
